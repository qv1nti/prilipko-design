...
// @access  Private
router.put("/profile", required, async (req, res) => {
  ...
  try {
    ...
    if (phone && !phoneRegex.test(phone)) {
      return res.status(400).json({ message: "Введіть коректний номер телефону у форматі +380XXXXXXXXX" });
    }
    ...
    if (phone && phone !== user.phone) {
      ...
      if (phoneTaken) {
        return res.status(400).json({ message: "Цей номер телефону вже використовується іншим користувачем" });
      }
    }
...
// @access  Private
router.put("/password", required, async (req, res) => {
